...
local function tile_group(cls, wa, orientation, fact, group)
    ...
    for c = group.first,group.last do
        ...
        size_hint = size_hint + cls[c].border_width*2 + 20
    ...
    for c = group.first,group.last do
        local i = c - group.first +1
        geom[width] = size - cls[c].border_width * 2 - 20
        geom[height] = math.floor(unused * fact[i] / total_fact) - cls[c].border_width * 2 - 20
        geom[x] = group.coord + 20
        geom[y] = coord + 20
        geom = cls[c]:geometry(geom)
        coord = coord + geom[height] + cls[c].border_width * 2 + 20
        unused = unused - geom[height] - cls[c].border_width * 2 - 20
        total_fact = total_fact - fact[i]
        used_size = math.max(used_size, geom[width] + cls[c].border_width * 2 + 20)
    end
...
local function do_tile(param, orientation)
    ...
    wa[width] = wa[width] - 20
    wa[height] = wa[height] - 20
--param.workarea
    local ncol = 2
--tag.getncol(t)
